    ...
    <main className="goa-root">
          ...
          {[1, 0, 2].map((idx) => {
            ...
            return (
              <div key={p.id} className={`goa-podium-slot ${cls}`} onClick={() => goToProfile(p.id)}>
                <PlayerAvatar avatarUrl={p.avatar_url} name={p.name} size={64} borderColor={idx === 0 ? "rgba(201,151,58,0.8)" : idx === 1 ? "rgba(192,192,192,0.7)" : "rgba(180,100,40,0.7)"} />
      ...
      <div className="goa-table-wrap">
        <div className="goa-header-row">
          <span className={`goa-col-head center ${sortKey === "rank" ? "active" : ""}`} onClick={() => handleSort("rank")}>#</span>
          <span className={`goa-col-head ${sortKey === "name" ? "active" : ""}`} onClick={() => handleSort("name")}>Name</span>
          ...
          return (
            <div key={p.id} className={`goa-row ${rowCls}`} onClick={() => goToProfile(p.id)}>
              ...
              <span className="goa-cell-name">
                <PlayerAvatar avatarUrl={p.avatar_url} name={p.name} size={26} />
                {p.name}
