...
const styles = `
  ...
  .goa-root {
    ...
    --gold-dark: #7A5A1A;
    --atl: #2AABB8;
    --atl-dim: rgba(26,107,122,0.25);
    --tit: #C42A3A;
    --tit-dim: rgba(122,26,42,0.25);
    --stone: #1C1A14;
    --stone-mid: #2A2720;
    --stone-lt: #3A3628;
    --border: rgba(201,151,58,0.28);
    --border-b: rgba(201,151,58,0.65);
    --txt: #F0E6C8;
    --muted: #A09070;
    --gain: #5DBB8A;
    --loss: #C44A4A;
    ...
    background:
      radial-gradient(ellipse 140% 55% at 50% 0%, rgba(26,107,122,0.18) 0%, transparent 52%),
      radial-gradient(ellipse 90% 40% at 50% 100%, rgba(122,26,42,0.12) 0%, transparent 50%),
  ...
  .goa-back-arrow { font-size: 0.75rem; }
  ...
  .goa-avatar {
    ...
    background: linear-gradient(135deg, var(--stone-lt), var(--stone-mid));
    display: flex;
    align-items: center;
  ...
  .goa-stat-tile.span2 { grid-column: span 2; }
  .goa-stat-lbl {
    font-family: 'Cinzel', serif;
    font-size: 0.58rem;
    letter-spacing: 0.15em;
  ...
  .goa-win-bar-wrap {
    margin-top: 0.5rem;
    height: 6px;
    background: rgba(196,42,58,0.3);
    border-radius: 3px;
    overflow: hidden;
  }
  .goa-win-bar {
    height: 100%;
    background: linear-gradient(90deg, var(--gain), rgba(93,187,138,0.7));
    border-radius: 3px;
  ...
  .goa-wl-sep { color: var(--muted); font-size: 0.7rem; }
  ...
  .goa-sec-head {
    ...
    background: linear-gradient(135deg, rgba(42,39,32,0.55), rgba(28,26,20,0.8));
    font-family: 'Cinzel', serif;
    font-size: 0.68rem;
    font-weight: 600;
    letter-spacing: 0.15em;
  ...
  .goa-match-badge.win { background: rgba(93,187,138,0.15); color: var(--gain); border: 1px solid rgba(93,187,138,0.3); }
  .goa-match-badge.loss { background: rgba(196,42,58,0.12); color: var(--loss); border: 1px solid rgba(196,42,58,0.28); }

  .goa-match-info {}
  .goa-match-date { font-family: 'Cinzel', serif; font-size: 0.6rem; letter-spacing: 0.06em; color: var(--muted); }
  .goa-match-teams { font-size: 0.8rem; color: var(--txt); margin-top: 0.1rem; }
  .goa-match-team-atl { color: var(--atl); }
  .goa-match-team-tit { color: var(--tit); }
  ...
  .goa-h2h-mmr { font-size: 0.58rem; color: var(--muted); font-weight: 400; }

  .goa-h2h-bars { display: flex; flex-direction: column; gap: 0.28rem; }
  .goa-h2h-bar-row { display: flex; align-items: center; gap: 0.4rem; }
  .goa-h2h-bar-lbl {
    font-family: 'Cinzel', serif;
    font-size: 0.52rem;
  ...
  .goa-h2h-bar-fill.with { background: linear-gradient(90deg, var(--atl), rgba(42,171,184,0.6)); }
  .goa-h2h-bar-fill.against { background: linear-gradient(90deg, var(--tit), rgba(196,42,58,0.6)); }
  .goa-h2h-bar-stat {
    font-family: 'Cinzel', serif;
    font-size: 0.58rem;
    color: var(--muted);
  ...
  .goa-h2h-no-data { text-align: center; font-style: italic; font-size: 0.8rem; color: var(--muted); padding: 1.2rem; }
...
export default function PlayerProfilePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [{ data: pData }, { data: allData }, { data: mData }] = await Promise.all([
        supabaseClient.from("players").select("id, name, mmr").eq("id", playerId).single(),
        supabaseClient.from("players").select("id, name, mmr").order("name"),
        supabaseClient
          .from("matches")
          .select(`
  ...
  return (
    <main className="goa-root">
      ...
      <div className="goa-hero">
        <div className="goa-avatar">{initials}</div>
