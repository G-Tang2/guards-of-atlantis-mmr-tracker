...
export async function POST(request: Request) {
  ...
  try {
    ...
    const sections = [
      discordContext && `Discord history (top priority — see instructions above):\n${discordContext}`,
      `Official rulebook:\n${RULEBOOK_TEXT}`,
      heroCardContext &&
    ...
    const colorFilterNote = askedColors.length
      ...
      : "";
    const cardInstruction = wantsCardDetail
      ? ` When you name a specific action card, refer to it by its exact name and focus on explaining/comparing it in prose — do not restate its exact color, level, initiative, or numeric values yourself, since the app automatically shows that card's exact data (sourced directly from the database, not from you) right alongside your answer.${colorFilterNote} If a question is about a hero whose cards aren't included below, say you don't have that hero's card details in this message rather than guessing.`
      : " This question doesn't appear to be asking about hero action card specifics, so no card data was included below — if it turns out you do need a card's exact details to answer well, say you don't have that hero's card data in this message rather than guessing at stats.";

    const systemInstruction = `You are a helpful assistant for the Guards of Atlantis II board game group. The group's own Discord message history below is your top-priority source — treat it as the primary source for how this group actually plays and talks about the game (house rules, opinions, running jokes, prior rulings), and lead with it over generic knowledge whenever it's relevant to the question. The official rulebook and hero card data below are secondary reference material for official rules and exact card specifics.${cardInstruction} For rules questions specifically: only state a rule, exception, or restriction if it is explicitly written in the rulebook or card text below — do not infer, speculate, or invent an exception based on theme, flavor text, "spirit of the rules", or assumed community consensus. If a general rule (e.g. what a Clear/Attack/Skill action can target) doesn't list an exception for a specific case, the general rule applies as written, even if the specific case sounds narratively special. If something isn't covered by the data below, say so honestly rather than making it up. You do not have access to the group's match history, player stats/MMR, or hero pick/win rates — if asked about those, say so rather than guessing.\n\n${sections.join("\n\n")}`;
