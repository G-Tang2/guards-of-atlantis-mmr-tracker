...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (wantsDetail) {
      ...
      priorityInstruction = ` This question is about hero strategy/playstyle rather than specific card facts — ground your reasoning in the hero's card data below so your advice is accurate, but answer in prose about her overall approach, strengths, and key plays rather than cataloging every card. The user didn't ask for card details, so no card-data box will be shown alongside this reply: don't enumerate her full card list or state exact numeric stats/tier/color as if they were verified facts. You may reference a specific card by its exact name when it helps illustrate a tip, but describe what it does qualitatively.`;
    } else {
